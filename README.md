...
- [3. Windows](#3-windows)
    ...
    - [3.3. Tools directory](#33-tools-directory)
        ...
        - [3.3.4. Link](#334-link)
    - [3.4. Configuration file](#34-configuration-file)
        - [3.4.1. Windows](#341-windows)
- [4. Preconfigured extensions (from configuration file)](#4-preconfigured-extensions-from-configuration-file)
    ...
    - [4.11. perl](#411-perl)
    - [4.12. C/C++](#412-cc)
    - [4.13. shell](#413-shell)
    - [4.14. go](#414-go)
    - [4.15. java](#415-java)
    - [4.16. php](#416-php)
    - [4.17. javascript](#417-javascript)
    - [4.18. css](#418-css)
    - [4.19. powershell](#419-powershell)
    - [4.20. docker](#420-docker)
    - [4.21. ansible](#421-ansible)
    - [4.22. puppet](#422-puppet)
    - [4.23. saltstack](#423-saltstack)
    - [4.24. markdown](#424-markdown)
    - [4.25. restructuredtext](#425-restructuredtext)
...
**_VSCode-Anywhere_** is a portable software. This means that you can use this tool anywhere (with an USB key for example) without privileges. The only thing you need for [installation](#312-process) is Internet (Internet is also needed if you want to [install](#331-install) or [update](#333-update) new components).

VSCode-Anywhere is preconfigured with features, you can modify or add your own features in the config file.

With traditionnal VSCode,for example if you want to developped in _Python_ language, you must to have Python installed with some modules like **flake8**. With VSCode-Anywhere you have just to enabled Python section in the config file and start the installation process.

VSCode-Anyware install for you (if *zeal_enabled* is set to _true_) the documentation of your language (with Zeal software). Always with Python example, Python2 and Python3 documentation will be downloaded add you will have access to this documentation accross your code.

Dependancies (like Python and modules in this example) will be installed with :
...
**Note :** if parameters are omitted, VSCode-Anywhere install and search configuration file in current shell path.
...
- PowerShell
- your device must be format with NTFS filesystem for installation on Windows

For windows, there are 3 configuration files available with all the same features configured but not the same features enabled :
...
- [VSCode-Anywhere_recommended.conf](Windows/VSCode-Anywhere_recommended.conf) : install VSCode-Anywhere with the recommended components. MSYS2 (configured with tmux-gigix, vim-gigix), Git, Zeal and VSCode (with theme, icons pack, fonts, and some extensions present in general section) will be installed. This is the recommended initial installation
- [VSCode-Anywhere_full.conf](Windows/VSCode-Anywhere_full.conf) : install VSCode-Anywhere with all sections enabled in the config file. Install can take a long time

There are multiples way to install :

- You can download configuration file [VSCode-Anywhere_minimal.conf](Windows/VSCode-Anywhere_minimal.conf) first, activate / modify / add your own sections that you need and after start the installation process
- You can install first with this configuration file [VSCode-Anywhere_recommended.conf](Windows/VSCode-Anywhere_recommended.conf) and on a second time enabled sections you need (like Python) and restart the installation process
...
- Replace _**C:\MyPATH**_ by your installation path or you can erase _**-path C:\MyPATH**_ if you want to install in your current shell path.
...
- After installation you can edit configuration file _**VSCode-Anywhere\Conf\VSCode-Anywhere.conf**_ and install your settings with run _**[VSCode-Anywhere\Tools\Install](#331-install)**_.
...
![install pacman upgrade](Screenshots/msys2_pacman_upgrade.png)
...
- base :
  ...
  - **Pzip_url** _type string_ : url to download 7-zip from a zip version (7za920.zip is the last version in zip format and it is used to extract new version in .7z format)
...
Installation execution order if enabled :
...
**minimal** is the minimal for running VSCode-Anywhere. This section install _Git for Windows_ and configure the environment.
...
**NOTE :** There is a problem resolved with function _cmd_post_ that copy file _msvc.64.linux.json_ to _msvc.64.intel.json_ in bin directory of this extension plugin. If this extension is updated you need to launch [Install](#331-install) or copy manually the file !
...
**NOTE :** not compatible with Windows (for the moment, hope it works in future version)
...
**Note :** You can replace with official extension below can be installed but it needed Puppet client (and it seems there is no portable version) :
