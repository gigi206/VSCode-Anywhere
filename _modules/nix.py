...
def __virtual__():
    ...
    """
    nixhome = os.path.join(os.path.expanduser("~"), ".nix-profile/bin/")
...
def _run(cmd):
    ...
    return __salt__["cmd.run_all"](
        cmd, env={"HOME": os.path.expanduser("~"), "NIXPKGS_ALLOW_UNFREE": 1}
    )
...
def _nix_channel():
    ...
    """
    nixhome = os.path.join(os.path.expanduser("~"), ".nix-profile/bin/")
    return [os.path.join(nixhome, "nix-channel")]
...
def install(*pkgs, **kwargs):
    ...
    if not pkgs:
        return "Please specify a package or packages to upgrade"
...
def list_pkgs(installed=True, attributes=True):
    ...
    return [
        ...
        for pkg in json.loads(out["stdout"]).values()
    ]
...
def channel_list():
    ...
    :return: Channels name and url
    :rtype: dict

    .. code-block:: bash

        salt '*' nix.channel_list
    """
    cmd = _nix_channel()
    cmd.append("--list")
    out = _run(cmd)
    ...
    return [
        {"name": channel.split()[0], "url": channel.split()[1]}
        for channel in out["stdout"].splitlines()
    ]
