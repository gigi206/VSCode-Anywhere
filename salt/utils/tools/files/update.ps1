...
if ($saltCurrentVersion -ne $saltTargetVersion) {
    ...
    & git pull --depth=100 origin vsc${saltTargetVersion}
    ...
    Set-Location ("{{ salt['vscode_anywhere.relpath'](salt['grains.get']('vscode-anywhere:tools:path'), salt['grains.get']('vscode-anywhere:saltstack:path')) }}" + '\bin\Lib\site-packages\salt-*')
    git reset --hard HEAD
    if (${env:VSCode_Anywhere_CI}) { git pull --quiet --ff-only origin vsc${saltTargetVersion} }
    else { git pull --ff-only origin vsc${saltTargetVersion} }
}
...
& .\vscode-anywhere.ps1 --retcode-passthrough --state-verbose=False state.apply salt/utils/update saltenv={{ saltenv }} sync_mods=all $Args --force-color
$exit = $?
