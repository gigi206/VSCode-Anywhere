...
{{ salt['vscode_anywhere.get_id'](sls) + ':install:{}:{}'.format(font, font_filename) }}:
  cmd.run:
    # CopyHere function can take 2 values 0x14 to copy fonts in C:\Users\gigi\AppData\Local\Microsoft\Windows\Fonts or 0x10 to copy in C:\Windows\Fonts
    ...
    - shell: powershell
    - unless:
      - powershell -Command { if ((Get-FileHash -Path "{{ salt['grains.get']('vscode-anywhere:apps:path') | path_join('vscode-anywhere', 'fonts', font, file) }}").Hash -ne (Get-FileHash -Path "{{ fonts_target | path_join(font_filename) }}").Hash) { exit 1 } }
