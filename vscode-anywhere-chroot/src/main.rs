...
fn bind_mount_directory(entry: &fs::DirEntry) {
    ...
    let mountpoint = PathBuf::from("/").join(entry.path().strip_prefix("/vscode-anywhere").unwrap());
...
fn run_chroot(nixdir: &Path, brewdir: &Path, rootdir: &Path, cmd: &str, args: &[String]) {
    ...
    // we pivot the old root to /vscode-anywhere. This is somewhat confusing, though.
    let mountpoint = rootdir.join("vscode-anywhere");
    fs::create_dir(&mountpoint)
        .unwrap_or_else(|_| panic!("failed to create {}", &mountpoint.display()));

    unistd::pivot_root(rootdir, &mountpoint).unwrap_or_else(|_| {
        panic!(
            "pivot_root({},{})",
            rootdir.display(),
            mountpoint.display()
    ...
    let root = PathBuf::from("/vscode-anywhere");
    let dir = fs::read_dir(&root).expect("failed to list /nix directory");
    for entry in dir {
        // println!("{:?}", entry);
        bind_mount_direntry(entry);
    }

    let brew_mountpoint = rootdir.join("home/linuxbrew");
    ...
    let brew_root = PathBuf::from("/vscode-anywhere/home");
    ...
    let brew_store = root.join(brewdir);
    mount(
        Some(&brew_store),
        "/home/linuxbrew",
        Some("none"),
    ...
    .unwrap_or_else(|_| panic!("failed to bind mount {} to /home/linuxbrew", brew_store.display()));
    ...
    let nix_store = root.join(nixdir);
    mount(
        Some(&nix_store),
        "/nix",
        Some("none"),
...
fn main() {
    ...
    if args.len() < 4 {
        eprintln!("Usage: {} <nixpath> <brewpath> <command>\n", args[0]);
        process::exit(1);
    }
